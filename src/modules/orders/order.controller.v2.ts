...
export class OrderControllerV2 {
  ...
  async getOrdersV2(@Query() query) {
    ...
    const [result, total] = await this.orderService.getOrdersV2(limit, page)
  ...
  async create(@Body() body: CreateOrderDto) {
    ...
    if (resultOrderDetail.length > 0) {
      const resultDeleteCart = await this.cartService.deleteCartByAccountId(
        body.account_id
      )
  ...
  async changeStatus(@Body() body: ChangeStatusDto) {
    ...
    const rawOrderDetails = await this.orderService.getOrderDetailsByOrderId(
      body.id
    )
