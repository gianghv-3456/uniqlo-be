...
export class CartService {
    constructor(
        ...
        private readonly variationRepositoty: Repository<Variation>,
    ...
    async getProductByVariationId(id: number) {
        return await this.variationRepositoty.findOne({
            where: { id: id },
            relations: ["product"],
        });
    }
    ...
    async findByInfo(data: any) {
        ...
        return await this.cartRepository.findOne({
            where: { account: { id: account_id }, variationId: variation_id },
        });
    }
    ...
    async create(cart: CreateCartDto) {
        ...
        try {
            const result = await queryRunner.manager.query(
                `INSERT INTO carts (account_id, quantity, variation_id) 
            VALUES($1, $2, $3) RETURNING *;`,
                [account_id, quantity, variation_id]
            );
