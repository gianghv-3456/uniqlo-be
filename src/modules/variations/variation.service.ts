...
export class VariationService {
    ...
    async getAllV2(limit, page) {
        ...
        return await this.variationRepository.findAndCount({
            skip: skip,
            take: limit,
        });
    }
    ...
    async create(variation: CreateVariationDto) {
        ...
        try {
            const result = await queryRunner.manager.query(
                `INSERT INTO variations (color, price, image, product_id, stock) 
            VALUES($1, $2, $3, $4, $5) RETURNING *;`,
                [color, price, image, product_id, stock]
            );
    ...
    async checkListQuantity(data: any) {
        ...
        const promises = data.map(async (item: any) => {
            ...
            if (!variation || variation.stock < item.quantity) {
                message.push(
                    `${variation.product.name}:${item.quantity - variation.stock}`
                );
                return false;
            }
        });
